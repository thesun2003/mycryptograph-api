...
class PoloniexAPI {
    ...
    public function get_all_balances($account = null) {
        return $this->query(
            array(
    ...
    public function get_all_available_balances() {
        return $this->query(
            array(
    ...
    public function get_current_rate($pair = 'USDT_BTC') {
        $ex_rates = $this->get_ticker($pair);
    ...
    public function get_total_balances() {
        ...
        foreach ($balances as $coin => $balance) {
            ...
            if ($amount > 0) {
                ...
                // convert to float
                $balance['available'] = floatval($balance['available']);
                $balance['onOrders'] = floatval($balance['onOrders']);
                $balance['btcValue'] = floatval($balance['btcValue']);

                // add more data
                $balance['tokenValue'] = floatval($amount);
